...
impl WebSocketClient {
    ...
    pub async fn connect(&self, config: WebSocketConfig) -> Result<()> {
        // 获取连接锁，确保同时只有一个连接操作
        let _lock: MutexGuard<'_, ()> = self.connection_mutex.lock().await;
        info!(
    ...
    pub async fn send_message(&self, data: serde_json::Value) -> Result<()> {
        ...
        match current_state {
            ConnectionState::Connected => {
                ...
                if let Some(sender) = sender.as_ref() {
                    ...
                    if pending.len() > 100 {
                        ...
                        warn!("Pending queue full, dropping oldest message {}", removed);
                    }
                    // 返回错误，让上层知道消息没有立即发送
                    Err(anyhow::anyhow!("连接未完全建立，将消息加入待发队列"))
                }
            }
            ConnectionState::Connecting | ConnectionState::Reconnecting => {
                ...
                pending.push(data);
                warn!(
                    "WebSocket is connecting, message queued(queue size: {})",
                    pending.len()
                );
                // 限制队列长度
                if pending.len() > 100 {
                ...
                Err(anyhow::anyhow!("WebSocket 正在连接中, 将消息加入待发队列"))
            }
            _ => {
                warn!("WebSocket not connected (state: {:?})", current_state);
                Err(anyhow::anyhow!(
                    "WebSocket 未连接 (状态: {:?})",
                    current_state
                ))
            ...
            match self.try_connect().await {
                ...
                Err(e) => {
                        ...
                        && attempts >= config.max_reconnect_attempts
                    {
                        ...
                        return Err(anyhow::anyhow!("到达最大连接重连次数"));
                    }
                    // 指数退避延迟
            ...
            tokio::spawn(async move {
                ...
                while heartbeat_active.load(Ordering::SeqCst) && !should_stop.load(Ordering::SeqCst)
                {
                    ...
                    if last_pong > 0 {
                        ...
                        if time_since_pong > timeout_ms {
                            let failures = consecutive_failures.fetch_add(1, Ordering::SeqCst) + 1;
                            warn!(
                                "Heartbeat timeout (consecutive failures: {}, last heartbeat {}ms ago)",
                                failures,
                                time_since_pong
                            );
    ...
    async fn send_pending_messages(&self) -> Result<()> {
        ...
        if let Some(sender) = sender.as_ref() {
            ...
            if !failed_messages.is_empty() {
                ...
                return Err(anyhow::anyhow!("一些待发消息发送失败"));
            }
            ...
            return Err(anyhow::anyhow!("消息发送器未就绪"));
        }
        Ok(())
    }
