...
/**
 * 音频压缩配置接口
 * @description 定义音频压缩的参数，包括声道数、采样率和比特率
 */
...
/**
 * 重采样音频到目标采样率
 * @param audioBuffer 原始音频缓冲区数据
 * @param targetSampleRate 目标采样率
 * @returns 重采样后的音频缓冲区数据
 */
 ...
 * @param audioBuffer 音频缓冲区数据
 * @param targetChannels 目标声道数，1为单声道，2为立体声
 * @returns 转换后的Int16Array音频样本数据
 */
...
/**
 * 将多声道音频混合为单声道
 * @param audioBuffer 多声道音频缓冲区
 * @returns 单声道音频数据，Float32Array格式
 */
...
/**
 * 使用lamejs将音频数据编码为MP3
 * @param samples 音频样本数据，Int16Array格式
 * @param config 压缩配置，包含声道数、采样率和比特率
 * @returns 编码后的MP3数据，每个MP3帧为Int8Array
 */
...
/**
 * 获取音频文件的基本信息
 * @param audioBuffer 音频文件的ArrayBuffer
 * @returns 包含音频时长、采样率、声道数和文件大小的对象
 */
