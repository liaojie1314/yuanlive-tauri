...
import { getUserDataRootAbsoluteDir } from "@/utils/PathUtils";
...
export const useScannerStore = defineStore(StoresEnum.SCANNER, () => {
  ...
  const startScan = async () => {
    ...
    try {
      const result = await invokeWithErrorHandler<DirectoryInfo>(
        TauriCommandEnum.GET_DIRECTORY_USAGE_INFO_WITH_PROGRESS,
        {
  ...
  const cancelScan = async () => {
    try {
      await invokeSilently(TauriCommandEnum.CANCEL_DIRECTORY_SCAN);
      console.log("扫描已取消");
    } catch (error) {
