...
import { getWasmMd5 } from "@/utils/Md5Utils";
import { getImageCache } from "@/utils/PathUtils";
import { removeTempFile } from "@/utils/TempFileManager";
import { isAndroid, isMobile } from "@/utils/PlatformUtils";
 ...
 * @returns 文件哈希
 */
const generateHashKey = async (fileObj: File, fileName: string) => {
...
export const useVoiceRecordRust = (options: VoiceRecordRustOptions = {}) => {
  ...
  const saveAudioToCache = async (audioBlob: Blob, duration: number) => {
    const getFileHashName = async (tempFileName: string) => {
      ...
      // 计算文件真正的资源哈希文件名
      const resourceFileName = await generateHashKey(audioFile, tempFileName);
