...
import { getOSType } from "@/utils/PlatformUtils.ts";
import { StorageKeyEnum } from "@/enums/index.ts";
...
export const getEnhancedFingerprint = async (): Promise<string> => {
  ...
  fingerprintPromise = (async () => {
    ...
    try {
      // 检查缓存是否有效
      const cachedData = localStorage.getItem(StorageKeyEnum.DEVICE_FINGERPRINT);
      if (cachedData) {
      ...
      if (fingerprint) {
        localStorage.setItem(
          StorageKeyEnum.DEVICE_FINGERPRINT,
          JSON.stringify({
            fingerprint,
            timestamp: Date.now()
          })
