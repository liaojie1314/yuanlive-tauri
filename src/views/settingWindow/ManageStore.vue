    ...
    <n-flex class="item" :size="16" vertical>
        ...
        <n-progress
          ...
          <n-flex vertical align="center" justify="center" :size="4" class="text-center">
            ...
            <span class="text-(12px #666 center)">
              {{ showDiskUsage ? t("setting.storage.usage") : t("setting.storage.fileScanProgress") }}
            </span>
          </n-flex>
        </n-progress>
      </n-flex>
      ...
      <n-flex justify="center" :size="8">
        <n-flex align="center" :size="8">
          ...
          <span class="text-(11px #666)">{{ t("setting.storage.appUsedSpace") }}</span>
        </n-flex>
        <n-flex align="center" :size="8">
          <div class="w-12px h-12px rounded-2px bg-[--warning-text]"></div>
          <span class="text-(11px #666)">{{ t("setting.storage.usedSpace") }}</span>
        </n-flex>
        <n-flex align="center" :size="8">
          <div class="w-12px h-12px rounded-2px bg-#7db1ac50"></div>
          <span class="text-(11px #666)">{{ t("setting.storage.freeSpace") }}</span>
        </n-flex>
      </n-flex>
      ...
      <n-flex vertical :size="8">
        <n-flex v-if="scanning" justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.currentFile") }}:</span>
        ...
        <n-flex v-if="scanning" justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.scanningProgress") }}:</span>
          <span class="text-(12px #666)">{{ scanningProgress.toFixed(2) }}%</span>
        </n-flex>

        <n-flex justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.processedFiles") }}:</span>
          <span class="text-(12px #666)">
            {{ t("setting.storage.processedFilesUnit", { count: scanProgress.files_processed }) }}
          </span>
        </n-flex>

        <n-flex justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.totalSize") }}:</span>
          <span class="text-(12px #666)">{{ formatBytes(scanProgress.total_size) }}</span>
        </n-flex>

        <n-flex v-if="diskInfo" justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.mountPoint") }}:</span>
          <span class="text-(12px #666)">{{ diskInfo.disk_mount_point }}</span>
        </n-flex>

        <n-flex v-if="diskInfo" justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.diskTotal") }}:</span>
          <span class="text-(12px #666)">{{ formatBytes(diskInfo.disk_total_space) }}</span>
        </n-flex>

        <n-flex v-if="diskInfo" justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.diskUsed") }}:</span>
          <span class="text-(12px #666)">{{ formatBytes(diskInfo.disk_used_space) }}</span>
        </n-flex>

        <n-flex v-if="diskInfo" justify="space-between">
          <span class="text-(12px #666)">{{ t("setting.storage.diskUsagePercent") }}:</span>
        ...
        <n-flex justify="space-between">
          ...
          <span class="text-(12px #666)">
            {{ t("setting.storage.elapsedUnit", { seconds: scanProgress.elapsed_seconds.toFixed(2) }) }}
          </span>
        </n-flex>
      </n-flex>
    </n-flex>
      ...
      <n-flex class="item" :size="16" vertical>
        ...
        <n-flex align="center" justify="space-between">
          ...
          <n-radio-group v-model:value="pathType">
            <n-radio value="default">{{ t("setting.storage.pathTypeDefault") }}</n-radio>
            <n-radio value="custom">{{ t("setting.storage.pathTypeCustom") }}</n-radio>
          </n-radio-group>
        </n-flex>
