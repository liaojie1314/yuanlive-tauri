      ...
      <n-flex class="item" :size="12" vertical>
        ...
        <n-flex align="center" justify="space-between">
          <n-flex vertical :size="8">
            ...
            <span class="text-(12px #909090)">{{ t("setting.shortcut.panelSwitchHint") }}</span>
          </n-flex>
          <n-flex align="center" :size="12">
            <n-tag
...
const shortcutConfigs: Record<string, ShortcutConfig> = {
  screenshot: {
    ...
    eventName: "screenshot-shortcut-updated",
    registrationEventName: "screenshot-shortcut-registration-updated",
...
const createSaveShortcutFunction = (config: ShortcutConfig) => {
  return async () => {
    try {
      console.log(`[Setting] 开始保存${config.displayName}: ${config.value.value}`);
      ...
      config.original.value = config.value.value;
      console.log(`[Setting] 已保存到 Pinia store`);

      // 通知主窗口更新快捷键（跨窗口事件）
      console.log(`[Setting] 发送 ${config.eventName} 事件到主窗口`);
      await emit(config.eventName, { shortcut: config.value.value });
      console.log(`[Setting] ${config.eventName} 事件已发送`);
      window.$message.success(t("setting.shortcut.shortcutUpdateResult", { name: config.displayName }));
    } catch (error) {
      console.error(`Failed to save ${config.key} shortcut:`, error);
      window.$message.error(t("setting.shortcut.shortcutSettingFailed", { name: config.displayName }));
      // 恢复原来的快捷键
...
const handleGlobalShortcutToggle = async (enabled: boolean) => {
  try {
    ...
    window.$message.success(enabled ? t("setting.shortcut.globalEnable") : t("setting.shortcut.globalDisable"));
  } catch (error) {
    console.error("Failed to toggle global shortcut:", error);
    window.$message.error(t("setting.shortcut.globalToggleFailed"));
    // 恢复原来的值
...
const saveSendMessageShortcut = async () => {
  try {
    ...
    window.$message.success(t("setting.shortcut.sendMessageUpdated"));
  } catch (error) {
    console.error("Failed to save send message shortcut:", error);
    window.$message.error(t("setting.shortcut.sendMessageFailed"));
    // 恢复原来的值
...
const createRegistrationListener = (config: ShortcutConfig) => {
  return listen(config.registrationEventName, (event: any) => {
    ...
    console.log(`[Setting] 收到${config.displayName}状态更新: ${shortcut} -> ${registered ? "已绑定" : "未绑定"}`);

    // 只有当前快捷键匹配时才更新状态
    if (shortcut === config.value.value) {
      console.log(`[Setting] ${config.displayName}匹配，更新状态为: ${registered ? "已绑定" : "未绑定"}`);
      config.isRegistered.value = registered;
    } else {
      console.log(`[Setting] ${config.displayName}不匹配，忽略状态更新`);
    }
  });
};
