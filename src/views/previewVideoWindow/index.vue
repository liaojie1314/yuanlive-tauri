  ...
  <div class="size-full bg-#000 relative flex flex-col select-none">
    ...
    <div data-tauri-drag-region class="z-9999 h-50px bg-#000 flex justify-center items-center gap-20px">
      ...
      <n-tooltip placement="top">
        ...
        {{ isPlaying ? t("preview.video.pause") : t("preview.video.play") }}
      </n-tooltip>
      ...
      <n-tooltip placement="top">
        ...
        {{ isMuted ? t("preview.video.unMute") : t("preview.video.mute") }}
      </n-tooltip>
...
const previousVideo = async () => {
  ...
  if (!currentVideoPath || currentVideoPath.startsWith("http")) {
    // 如果是网络视频，使用原有逻辑
    currentIndex.value--;
    showVideoTip(t("preview.video.tipPlayingIndex", { index: currentIndex.value + 1 }));
  } else {
    // 如果是本地视频，从文件夹获取视频列表
    ...
    if (folderVideos.length > 0) {
      ...
      if (currentVideoIndex > 0) {
        ...
        showVideoTip(t("preview.video.tipPlayingPrevious"));
      } else {
        showVideoTip(t("preview.video.tipFirst"));
        return;
      }
    } else {
      // 如果获取文件夹视频失败，使用原有逻辑
  ...
  if (!currentVideoPath || currentVideoPath.startsWith("http")) {
    ...
    if (folderVideos.length > 0) {
      ...
      if (currentVideoIndex < folderVideos.length - 1) {
        ...
        showVideoTip(t("preview.video.tipPlayingNext"));
      } else {
        showVideoTip(t("preview.video.tipLast"));
        return;
      }
    } else {
      // 如果获取文件夹视频失败，使用原有逻辑
